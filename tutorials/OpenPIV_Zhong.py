...
def process_single(image1, image2, num, interrogation_window_size, pixel_to_mm):
    ...
    overlap=int(interrogation_window_size/2)
    u, v, sig2noise = pyprocess.extended_search_area_piv( frame_a, frame_b, 
        window_size=interrogation_window_size, overlap=overlap, dt=0.01, search_area_size=interrogation_window_size*2, sig2noise_method='peak2peak' )

    print(u,v,sig2noise)
    #get coordinate get the center of each interrogation window
    x, y = pyprocess.get_coordinates( image_size=frame_a.shape, search_area_size=interrogation_window_size*2, overlap=int(interrogation_window_size/2) )
    #Vectors below a certain threshold are substituted by NaN, bascially masking
    flags_s2n = validation.sig2noise_val(sig2noise, threshold = 0.5 )
    ...
    display_vector_field(
        ...
        window_size=interrogation_window_size,
        scale=1, 
...
def particle_masking():
    path = files('openpiv') / "data" / "test1"/"Cropped" 
    list_image = natsorted(os.listdir(path))
    print(list_image)
    #pre-processing
    for i,_ in enumerate(list_image):
...
def process_video_to_frame():

    # path of video file
    video_path = r'data/test1/vid_2000-08-08_04-40-22.mp4'
    ...
    # Calculate the number of frames needed to achieve the desired phase
    frames_per_period = int(fps * period)
    print('frames per period:',frames_per_period)
    frames_per_desired_phase = int(frames_per_period / 360 * desired_phase)
    print('frames per desired cycle:',frames_per_desired_phase)
    ...
    for i in range(frame_count):
        ret, frame = video.read()
        #output for same phase
        if ret and (i % frames_per_period == frames_per_desired_phase):
            frames.append(frame)
            # cv2.imwrite(image_path, frame)
...
# marker_locations = []
# frameSize = []
# (marker_locations, frameSize)= ArucoMarker(os.path.join(path,list_image[0]))
# print(marker_locations)
...
# distance_in_pixels = right_marker_x - left_marker_x 
# distance_in_pixels_after_masking = distance_in_pixels
# print(distance_in_pixels)
# distance_in_mm = 17.988
# particle_size_in_mm = 0.476
# input_path = r'data/test1/Cropped'
# input_list_image = os.listdir(input_path)
# # Calculate the number of particles in one window
# particles_per_window = 15

# # Calculate the size of the interrogation window in pixels
# interrogation_window_size = round((distance_in_pixels_after_masking/distance_in_mm)*(particles_per_window*particle_size_in_mm))
# interrogation_window_size = interrogation_window_size + (interrogation_window_size % 2)
# pixel_to_mm = distance_in_mm/distance_in_pixels_after_masking
# print(interrogation_window_size)
# print(pixel_to_mm)


# for i,_ in enumerate(os.listdir(input_path)[:-1]):
#     process_single(input_list_image[i], input_list_image[i+1],i,interrogation_window_size, pixel_to_mm )
# print("fini")
